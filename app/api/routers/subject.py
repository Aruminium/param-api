...
async def list_subject(db: AsyncSession = Depends(get_db), current_user: str = Depends(oauth2.get_current_user)):
  if current_user:
    return await subject_crud.get_subjects(db)

@router.post("/", response_model=subject_schema.CreateSubjectResponse)
async def create_subject(subject_body: subject_schema.CreateSubject, db: AsyncSession = Depends(get_db), current_user: str = Depends(oauth2.get_current_user)):
...
async def update_subject(subject_id: int, subject_body: subject_schema.CreateSubject, db: AsyncSession = Depends(get_db), current_user: str = Depends(oauth2.get_current_user)):
...
async def delete_subject(subject_id: int, db: AsyncSession = Depends(get_db), current_user: str = Depends(oauth2.get_current_user)):
  subject = await subject_crud.get_subject_detail(db, subject_id=subject_id)
  if subject is None:
    raise HTTPException(status_code=404, detail="Subject not found")
